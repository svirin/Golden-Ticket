...
namespace GoldenTicket.DataProxy.Parse
{
    public class ConcertDataProvider : IConcertDataProvider<ParseObject>
    {
        ...
        public IEnumerable<Concert> GetSuggestToUser(User user)
        {
            var suggestProvider = DI.Factory.GetInstance<ISuggestDataProvider<ParseObject>>();
            
            var suggests = suggestProvider.GetSuggestByUser(user);

            var concertsIds = suggests.Select(item => item.ConcertId);

            var concerts = GetByIds(concertsIds);
